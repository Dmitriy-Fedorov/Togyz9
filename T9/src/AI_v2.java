...
public class AI_v2 {
	...
	AI_v2(Desk player_1,Desk player_0,Kazan Kazan_1,Kazan Kazan_0){
		player[1] = player_1;
		player[0] = player_0;
		Kazan[1] = Kazan_1;
		Kazan[0] = Kazan_0;
	...
	int dumbTreePlay_v2(int player_,int dummy_depth,int dummy_width,boolean echo){
		
		int[] scoreTemp = new int[9],top3 = new int[3];
		int tempDest, scoreTempMax, tophod;
		
		for(int i=0;i<9;i++){
			tempDest = player[player_].getDestinationCELL(i,echo,"1to9 format");
			//System.out.println("tempDest"+i+" :"+tempDest);
			if(tempDest>0){
				scoreTemp[i] = this.getVirtScoreTemp(0, tempDest);
