...
public static void main(String[] args) {
	...
	*/
	...
	while(!Kazan[0].checkWin() && !Kazan[1].checkWin()
		...
		if(hod%2==0){
			n = print_T9.selectPlayer(hod, "random", player[1],player[0],Kazan[1],Kazan[0]);
			//player[hod%2].getDestinationCELL(n);
		}else{
			n = print_T9.selectPlayer(hod, "GO", player[1],player[0],Kazan[1],Kazan[0]);
			//player[hod%2].getDestinationCELL(n);
		}
		...
		try {
			AI_v0.generateArray(hod,hod_number,n,player[1],player[0],Kazan[1],Kazan[0]);
		} catch (Exception e) {
